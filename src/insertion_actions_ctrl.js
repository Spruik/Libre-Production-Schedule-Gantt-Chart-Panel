...
function addListenersForOrderForm () {
  $(document).on('click', 'button#product-schedule-gantt-chart-order-form-submitBtn', (e) => {
    const data = $('form#product-schedule-gantt-chart-order-form').serializeArray()
    submitOrder(data)
  })
...
function updateDuration (qty, rate) {
  if (qty !== '' && rate !== '') {
    const durationHrs = Number(parseFloat(qty).toFixed(2)) / Number((parseFloat(rate) * 60).toFixed(2))
