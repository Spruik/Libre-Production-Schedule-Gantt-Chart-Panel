...
function findAffectedOrdersInLineChangedCase (allData, order, isOriginal) {
  ...
  const ordersBeingAffected = ordersWithSameLineAndDate.filter((or) => or.startTime > order.startTime)
  if (_isInsertingBefore) {
    if (!isOriginal) {
...
function findAffectedOrders (allData, droppingOrder, targetingOrder, isMovingForward) {
  ...
  const ordersBeingAffected = ordersWithSameLineAndDate.filter((order) => {
    const targetingStartTime = moment(targetingOrder.startTime)
    const droppingStartTime = moment(droppingOrder.startTime)
    if (isMovingForward) {
